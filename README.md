...
一个跑在 AgentTeams 框架上的多智能体代码审查系统：用户直接对接协调官（orchestrator，团队 Leader），由其统一编排 `coding-review` 团队下的 4 个业务智能体——审查员 / 修复员 / 测试验证员并行工作，通过共享状态板对齐上下文，自动完成一次代码审查的端到端闭环。（Manager 为 AgentTeams 平台默认 Agent，仅负责密钥 / 令牌保管，不参与团队管理。）

| 角色 | 名称 | 职责 |
| --- | --- | --- |
| 协调官（团队 Leader / 用户入口） | orchestrator | 直接接收任务、团队内编排：拆解子任务、分派成员、跟踪进度、失败重试、汇总报告 |
| 审查员 | reviewer | 发现缺陷 / 安全隐患，输出结构化审查报告 |
| 修复员 | fixer | 按审查意见与缺陷模式库定位并修改代码 |
| 测试验证员 | tester | 通过 mock 网关调用执行测试，确认无回归 |

> *注：Manager 是 AgentTeams 平台默认 Agent，仅负责密钥 / 令牌保管，不进入「协调官 → 执行员」这条管理链。*
...
```
            ┌──────────────────────────────────┐
            │  orchestrator（协调官 / Leader）    │  用户直接对接 · 团队内拆解 / 分派 / 重试 / 汇总
            └──────────────┬───────────────────┘
       ┌──────────────────┼──────────────────┐
       ▼                  ▼                  ▼
   ...
   （Manager 为平台默认 Agent，仅保管密钥 / 令牌，不进入本管理链）
```
...
3. 把任务直接发给 **协调官（orchestrator）**：在 orchestrator 的对话 / 房间里直接发需求，例如：
   > 请审查这段代码：<粘贴代码或仓库地址>
   orchestrator 会把它拆解成「审查 → 修复 → 测试」子任务，并和 reviewer / fixer / tester 接力完成。
